...
def lambda_handler(event, context):
    ...
    output_key = 'output/saurabh_processed70_to_80.csv'

    input_file_path = '/tmp/saurabh.csv'
    output_file_path = '/tmp/saurabh_processed70_to_80.csv'

    # Download the input file from S3
    s3_client.download_file(bucket, input_key, input_file_path)

    # Process and write rows 70 to 80 to a new file
    new_rows = []
    ...
    response = {
        "statusCode": 200,
        "body": "\"Processed rows 70 to 80 and saved to output folder\"",
        "success": True
    }
    return response