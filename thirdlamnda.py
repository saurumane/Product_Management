...
def lambda_handler(event, context):
    ...
    output_key = 'output/saurabh_processed20_to_30.csv'

    input_file_path = '/tmp/saurabh.csv'
    output_file_path = '/tmp/saurabh_processed20_to_30.csv'

    # Download the input file from S3
    s3_client.download_file(bucket, input_key, input_file_path)

    # Process and write rows 20 to 30 to a new file
    new_rows = []
    ...
    response = {
        "statusCode": 200,
        "body": "\"Processed rows 20 to 30 and saved to output folder\"",
        "success": True
    }
    return response
