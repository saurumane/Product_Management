...
def lambda_handler(event, context):
    ...
    output_key = 'output/saurabh_processed50_to_60.csv'

    input_file_path = '/tmp/saurabh.csv'
    output_file_path = '/tmp/saurabh_processed50_to_60.csv'

    # Download the input file from S3
    s3_client.download_file(bucket, input_key, input_file_path)

    # Process and write rows 50 to 60 to a new file
    new_rows = []
    ...
    response = {
        "statusCode": 200,
        "body": "\"Processed rows 50 to 60 and saved to output folder\"",
        "success": True
    }
    return response