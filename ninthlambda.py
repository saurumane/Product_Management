...
def lambda_handler(event, context):
    ...
    output_key = 'output/saurabh_processed80_to_90.csv'

    input_file_path = '/tmp/saurabh.csv'
    output_file_path = '/tmp/saurabh_processed80_to_90.csv'

    # Download the input file from S3
    s3_client.download_file(bucket, input_key, input_file_path)

    # Process and write rows 80 to 90 to a new file
    new_rows = []
    ...
    response = {
        "statusCode": 200,
        "body": "\"Processed rows 80 to 90 and saved to output folder\"",
        "success": True
    }
    return response
