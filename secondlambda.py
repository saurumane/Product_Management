...
def lambda_handler(event, context):
    ...
    output_key = 'output/saurabh_processed10_to_20.csv'

    input_file_path = '/tmp/saurabh.csv'
    output_file_path = '/tmp/saurabh_processed10_to_20.csv'

    # Download the input file from S3
    s3_client.download_file(bucket, input_key, input_file_path)

    # Process and write rows 10 to 20 to a new file
    new_rows = []
    ...
    response = {
        "statusCode": 200,
        "body": "\"Processed rows 10 to 20 and saved to output folder\"",
        "success": True
    }
    return response
